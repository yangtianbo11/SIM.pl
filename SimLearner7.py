...
def startLog(frame, data=None):
    if DEBUG:
        with open('training_log1.txt', 'a') as f:
            ...
            print(f'Game({num_vertices}_{order}_{num_episodes}_{num_eval_episodes}_Entering function: {function_name} with Data: {data}')
            # Restore stdout back to the console
            sys.stdout = sys.__stdout__
            return f


def endLog(frame, f, data=None):
    if DEBUG:
        with open('training_log1.txt', 'a') as f:
            # Redirect stdout to the file
            sys.stdout = f
            # frame = inspect.currentframe()
            function_name = inspect.getframeinfo(frame).function
            print(f'Game({num_vertices}_{order}_{num_episodes}_{num_eval_episodes}_Leaving function: {function_name} with Data: {data}')
            # Restore stdout back to the console
            sys.stdout = sys.__stdout__
...
class SimplicialComplexGame:
    ...
    def step(self, action):
        ...
        f = startLog(inspect.currentframe())
        player = self.turn
        ...
        if player == 1:
            ...
            if self.lose(edges, self.player1):
                ...
                filename = f"gameResults/Player1_{num_vertices}_{order}_{num_episodes}_{num_eval_episodes}_{timestamp}.png"
...
def main():
    ...
    if not os.path.exists(folder_path):
        ...
        # Iterate over the files and directories and remove them
